...
// camera defintion. (View point)
Camera get_view_point()
{
	Camera view_point{};
	uintptr_t location_pointer = readd<uintptr_t>(cache::uworld + 0x120); 
	uintptr_t rotation_pointer = readd<uintptr_t>(cache::uworld + 0x130); 
	FNRot fnrot{};
...
//	view_point.fov = readd<float>(cache::local_pawn + 0x4AC); //0x3AC
	return view_point;
}
// world to screen
Vector2 w2s(Vector3 world_location)
{
...
{
	uintptr_t bone_array = readd<uintptr_t>(mesh + BONE_ARRAY);
	if (bone_array == 0) bone_array = readd<uintptr_t>(mesh + BONE_ARRAY_CACHE);
	FTransform bone = readd<FTransform>(bone_array + (bone_id * 0x60));
	FTransform component_to_world = readd<FTransform>(mesh + COMPONENT_TO_WORLD);
...
{
	auto Seconds = readd<double>(cache::uworld + 0x140);
	auto LastRenderTime = readd<float>(mesh + 0x30C);
